...
final class TextFilterTest extends FilterTestCase
{
    ...
    public function test_can_set_text_filter_wireable_live(): void
    {
        ...
        $this->assertSame('wire:model.blur="appliedFilters.active"', $filter->getWireMethod('appliedFilters.'.$filter->getKey()));

        $filter->setWireLive();

        $this->assertSame('live', $filter->getWireableMethod());
        $this->assertSame('wire:model.live="appliedFilters.active"', $filter->getWireMethod('appliedFilters.'.$filter->getKey()));

        $filter->setWireDefer();

        $this->assertSame('defer', $filter->getWireableMethod());
        $this->assertSame('wire:model="appliedFilters.active"', $filter->getWireMethod('appliedFilters.'.$filter->getKey()));
        ...
        $this->assertSame('wire:model.live.debounce.250ms="appliedFilters.active"', $filter->getWireMethod('appliedFilters.'.$filter->getKey()));

        $filter->setWireDebounce(500);

        $this->assertSame('live.debounce.500ms', $filter->getWireableMethod());
        $this->assertSame('wire:model.live.debounce.500ms="appliedFilters.active"', $filter->getWireMethod('appliedFilters.'.$filter->getKey()));
