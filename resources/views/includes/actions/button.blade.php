<a {{ $attributes->merge()
            ->class([
                ...
                'justify-center text-center items-center inline-flex space-x-2 rounded-md border shadow-sm px-4 py-2 text-sm font-medium focus:ring focus:ring-opacity-50' => $isTailwind4 && ($attributes['default-styling'] ?? true),
                'focus:border-indigo-300 focus:ring-indigo-200' => $isTailwind4 && ($attributes['default-colors'] ?? true),
        ...
        @if($action->hasIcon() && $action->getIconRight())
            ...
            <i {{ $action->getIconAttributes()
                    ->class([
                        ...
                        'ml-1 '. $action->getIcon() => $isTailwind4,
                    ])
                    ->except(['default','default-styling','default-colors'])
                }}
            ></i>
        @elseif($action->hasIcon() && !$action->getIconRight())
            <i {{ $action->getIconAttributes()
                    ->class([
                        ...
                        'mr-1 '. $action->getIcon() => $isTailwind4,
